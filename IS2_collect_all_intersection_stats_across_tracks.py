...
Created on Tue Feb 11 08:31:32 2025
...
# search radius used in 'find_IS_intersections.py' 
radius = 500

# define directories
PROJ_DIR = Path('/home/cat/onedrive/work/PhD/belgica_bank_study')
DATA_DIR = PROJ_DIR / 'data' 
IS2_DIR = DATA_DIR / 'IS2' / 'ATL10' / f'intersections_across_beams_radius{radius}'

RESULTS_DIR = PROJ_DIR / 'results' / 'altimetry' / 'IS2' / 'cross-over_study' / f'intersections_across_beams_radius{radius}'
Path(RESULTS_DIR).mkdir(parents=True, exist_ok=True)

outfile_path = IS2_DIR / f'all_intersections_stats_across_beams_radius{radius}_dataframe.pkl'

# read in Belgica Bank fast ice ROIs --> for plotting
fastice_roi_path = DATA_DIR / 'shapefiles' / 'BB_fastice_ROI_4326.shp'
fastice_ROI = gpd.read_file(fastice_roi_path)
...
all_dict_filepaths = glob.glob((IS2_DIR/'processed_*.pkl').as_posix())
...
for filepath in all_dict_filepaths:
    ...
    for t_df, query_dfs in zip(t_dfs_list, query_dfs_list):
        ...
        # if there are different timestamps (i.e. different granules intersecting) --> split up into multiple crossovers
        q_df['date'] = pd.to_datetime(q_time).date
        q_unique_dates = np.unique(q_df['date'])
        #t_df['date'] = pd.to_datetime(t_time).date
        
        # if there are multiple querys that intersect target track, loop through each one separately
        if len(q_unique_dates) > 1:
            ...
            for unique_date in q_unique_dates:
                ...
                for q_beam in q_beams:
                    ...
                    t_fb = t_df_subset.freeboard_v4.values
                    
                    q_xy = [q_df_subset_beam.point_x.values, q_df_subset_beam.point_y.values]
                    q_fb = q_df_subset_beam.freeboard_v4.values
                    ...
                    map_outdir = RESULTS_DIR / 'crossovers_on_map'
                    Path(map_outdir).mkdir(parents=True, exist_ok=True)
                    crossing_map_outpath = map_outdir / f'crossing_{t_date_formatted}_{q_date_formatted}_beam_{t_beam}.png'
                    ...
                    #ax.set_extent([-10, -6, 80.5, 78.5], ccrs.PlateCarree()) 
                    
                    ax.add_geometries(list(fastice_ROI['geometry']), crs=ccrs.PlateCarree(), facecolor='None', edgecolor='b', alpha=0.8)
                    ...
                    hist_outdir = RESULTS_DIR / 'crossover_histograms'
                    Path(hist_outdir).mkdir(parents=True, exist_ok=True)
                    crossing_hist_outpath = hist_outdir / f'hist_crossing_{t_date_formatted}_{q_date_formatted}_beams_{t_beam}_{q_beam}.png'
                    
                    plt.figure(figsize=(10,8))
                    ax = plt.axes()
                    
                    plt.hist(t_fb, density=True, bins=15, histtype='step', label='Target granule')
                    plt.hist(q_fb, density=True, bins=15, histtype='step', label='Query granules')
                    
                    # add textbox with target stats in histogram plot
                    textstr_t = '\n'.join((
                    r'$mean=%.2f$' % (t_fb_avg, ),
                    r'$IQR=%.2f$' % (t_iqr, ),
                    r'$roughness=%.2f$' % (t_rough_avg, )))
                    
                    # these are matplotlib.patch.Patch properties
                    props_t = dict(boxstyle='round', facecolor='lightblue', alpha=0.7)
                    
                    # place a text box in upper left in axes coords
                    ax.text(0.75, 0.85, textstr_t, transform=ax.transAxes, fontsize=14,
                            verticalalignment='top', bbox=props_t)
                    
                    # add textbox with query stats in histogram plot
                    textstr_q = '\n'.join((
                    r'$mean=%.2f$' % (q_fb_avg, ),
                    r'$IQR=%.2f$' % (q_iqr, ),
                    r'$roughness=%.2f$' % (q_rough_avg, )))
                    
                    props_q = dict(boxstyle='round', facecolor='orange', alpha=0.7)
                    ax.text(0.75, 0.70, textstr_q, transform=ax.transAxes, fontsize=14,
                            verticalalignment='top', bbox=props_q)
                            
                    plt.xlabel('Laser freeboard [m]')
                    plt.ylabel('PDF [-]')
                    plt.title(f'Intersection {t_date_formatted} and {q_date_formatted}')
                    plt.legend(loc='upper right')
                    
                    plt.tight_layout()
                    plt.savefig(crossing_hist_outpath)
        ...
        # if there is only a single intersecting track
            ...
            for q_beam in q_beams:
                ...
                q_fb = q_df_subset.freeboard_v4.values 
                ...
                # if either target or query dataframes have too few samples, skip
                if (len(t_df_subset) < 100) or (len(q_df_subset) < 100):
                    continue
